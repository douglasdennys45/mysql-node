import { PostgresConnector, ConnectionFailure, PgUser } from '@/infrastructure/database/postgres'
...
describe('PostgresConnector', () => {
  ...
  it('should return ConnectionFailure on openTransaction if connection is not found', async () => {
  ...
  it('should return ConnectionFailure on getRepository if connection is not found', async () => {
    expect(getRepositorySpy).not.toHaveBeenCalled()
    expect(() => sut.getRepository(PgUser)).toThrow(new ConnectionFailure())
  })
})